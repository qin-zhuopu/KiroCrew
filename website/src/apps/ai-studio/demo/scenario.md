...
### 主线 `main-membership-points`「会员积分系统」（23 步）
...
17. main-17 同点第三拍：环住**右段**「重生成差异」——v4 为这一点带出的行，
    ...
    组携带任何文档行，空也是数据
18. main-18 点顶栏「开始开发」（ACP-735，第四个真实点击步，验收文档步骤
    12）：落位到 after_fix 快照 main-015——开发记录面板亮起，头部徽章把它
    锚定到冻结设计 `v4 · graph@distill-v3`（追溯的起点），「任务生成」阶段
    进行中。四阶段固定为 任务生成→编码实现→测试验证→构建打包，**进度推进
    靠快照帧，不靠计时器**
19. main-19 开发进行中（验收文档步骤 13）：载入过程帧 main-016——前两阶段
    已完成并各自带一句事实摘要（任务清单自五点需求；实现带着 7 天有效期），
    进行中阶段没有摘要（`check_devrun`：完成⇒摘要非空，未完成⇒摘要必须为
    空）。此时没有产物、没有体验入口——结果只能在完成后出现
20. main-20 开发完成（验收文档步骤 14）：载入完成帧 main-017——四阶段全部
    完成，产物三件套（测试报告/构建产物/运行时）齐，可运行版本 0.4.0。
    「打开可运行版本」按钮只在宣告这一跳转的下一步（main-21）出现——与
    沉淀按钮从不出现在已落位帧同一规矩
21. main-21 打开可运行版本（验收文档步骤 15，第五个真实点击步）：点击只在
    **演示 overlay 内部**把画面切到内置快照组件 RunPreviewScreen——不起任何
    服务器、容器或真实部署（工单硬约束）。体验页展示的功能清单恰好等于图
    谱需求节点的标签（生成器按 GRAPH_DISTILLED 校验 `runPreview.lines`）：
    可运行的东西不多不少，正是结构化设计承诺过的
22. main-22 看全过程历史（ACP-736，验收文档步骤 16）：工作台底部亮起项目
    历史时间线（`ProjectHistoryView`，纯 props 组件）——本轮六个事件：编辑、
    提交、发版、沉淀、开发、运行，各是一类事实、各有图标。每个事件带
    `ref`（它产出的画面）与 `links`（回指前因）；链是快照里的数据，生成器
    `check_history` 校验：kind 只许六种封闭枚举（不引入 AI 来源枚举——DAG
    显式不做验收文档步骤 3-4，文档修改一律按人工来源建模）、每条链接必须
    指向更早的事件（追溯沿因果往回走）、每个 `ref` 必须命名剧本里某一步
    真实展示过的快照。点任一节点「回到当时画面」= 进入展示该快照的那一步
    （加载，禁止反向计算）——从运行节点一路点回最初那次修改，验收文档
    步骤 16 的追溯在数据里成立
23. main-23 点「继续设计」（验收文档步骤 17，第六个真实点击步）：落位到
    after_fix 快照 main-020——编辑器干净地站在上一轮最终设计 v4 上（版本、
    图谱、开发记录、六事件时间线一件不少），可直接开始下一轮修改。闭环不
    是重开一局，是在成果上续写
...
已重生成 --开始开发--> 开发中(记录锚定 vN+1 · graph@图版；四阶段推进靠快照帧)
开发中 --阶段全部完成--> 开发完成(产物三件套 + 可运行版本；完成前无产物无入口)
开发完成 --打开可运行版本--> 体验(overlay 内切换到内置快照组件；无服务器/容器)
体验 --打开项目历史--> 可追溯(时间线六事件：编辑/提交/发版/沉淀/开发/运行，
                        每条链接回指更早事件；点节点=加载它命名的快照)
可追溯 --继续设计--> 新一轮(编辑器干净落在上一轮最终设计上；历史全部保留)
草稿 --恢复某条自动保存--> 草稿(当前内容=该条)
已提交 --编辑--> 草稿
```

图谱、发版、沉淀、重生成与开发只在携带它们的世界里参与状态机（主线快照全部带
`graph`，主线末尾几步带 `release`/`generatedFiles`/`distillation`/
`regeneration`/`diffGroups`/`devRun`/`runPreview`/`history`/`newRound`，
分支线都没有）：
...
图谱高亮不可能互相撒谎。开发运行（`devRun`）在每一帧都全量自检
（`check_devrun`）：设计版本徽章必须等于 `重生成版本 · graph@沉淀 id`、四
阶段名固定、完成⇔摘要非空、产物与可运行版本只能在全部完成后出现、体验页
功能清单必须逐字等于图谱需求节点标签——「可运行」不是嘴上说的。全过程历史
（`history`）同样每帧自检（`check_history`）：kind 封闭在
edit/commit/release/distill/dev/run 六种、事件 id 唯一、每条链接指向更早
事件、每个 `ref` 命名剧本某步真实展示的快照（可跳转集 = 各步 fixture ∪
afterFix）、`newRound` 帧必须带 `history`——时间线不是一幅画，是数据。
图谱视图、代码预览、沉淀面板、开发面板、体验页与历史时间线都是业务侧纯
props 组件（`GraphView.tsx` / `CodeGenView.tsx` / `DistillPanel.tsx` /
`DevRunView.tsx` / `ProjectHistoryView.tsx`），
发版/沉淀/开发按钮（`ReleaseControl.tsx`）真实路径不渲染——后端还没有图谱/
发版/生成/沉淀/开发端点，不放假功能。
...
| 验收文档「总体验收标准」 | 演示里的落点 |
|---|---|
| 1. 业务用户不懂图谱/状态机/SDD 也能走完主路径 | 主线 23 步全程只出现用户语言（提交/发版/沉淀/开发/继续设计），术语藏在提示文案背后 |
| 2. 修改、提交、发版、沉淀、开发是五类可追溯事实 | main-22 时间线六事件各一类图标；`check_history` 校验 kind 封闭 + 链接回指更早事件 |
| 3. 每步完成都有"刚完成什么/下一步做什么" | 每步 `startFrom`/`userSees`/`hint` 三段由生成器从快照推导 |
| 4. 文档修改经沉淀可由结构化数据重生成 | main-13 图谱吸收候选 → main-14 从「设计事实」重生成 v4（`check_regen` 逐字校验） |
| 5. 成组历史：用户 Diff + 结构化变化 + 重生成 Diff | main-15/16/17 三连拍逐段环住同一业务点的三段 |
| 6. 直接结构化编辑不伪造用户文档修改 | **N/A**——本演示不做验收文档步骤 3-4（DAG 显式排除），全部文档修改按人工来源建模，不存在直接结构化编辑入口，也就没有可伪造的路径 |
| 7. 开发结果可追溯到沉淀/发版/提交/修改 | main-18 记录锚定 `v4 · graph@distill-v3`；main-22 从运行节点沿链回指到最初修改 |
| 8. 从运行结果发起下一轮设计成闭环 | main-23 点「继续设计」干净落在 v4 上，历史六事件保留 |
