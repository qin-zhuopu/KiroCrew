...
### 主线 `main-membership-points`「会员积分系统」（9 步）
...
图谱只在携带它的世界里参与状态机（主线快照全部带 `graph`，分支线没有）：
节点数与「本次新增数」是 before/after 的可选字段，`graphDelta` 必须等于
前后快照图谱的集合差（生成器拒绝不一致）。图谱视图是业务侧纯 props 组件
（`GraphView.tsx`，SVG 自绘），真实路径暂无入口——后端还没有图谱端点，
不放假功能。
