...
### 主线 `main-membership-points`「会员积分系统」（21 步）
...
`regeneration`/`diffGroups`/`devRun`/`runPreview`，分支线都没有）：
...
功能清单必须逐字等于图谱需求节点标签——「可运行」不是嘴上说的。图谱视图、
代码预览、沉淀面板、开发面板与体验页都是业务侧纯 props 组件
（`GraphView.tsx` / `CodeGenView.tsx` / `DistillPanel.tsx` / `DevRunView.tsx`），
