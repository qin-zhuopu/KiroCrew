...
### 主线 `main-membership-points`「会员积分系统」（13 步）
...
图谱、发版与沉淀只在携带它们的世界里参与状态机（主线快照全部带 `graph`，
主线末尾几步带 `release`/`generatedFiles`/`distillation`，分支线都没有）：
