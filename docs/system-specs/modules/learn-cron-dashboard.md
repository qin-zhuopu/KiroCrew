...
store, read on the loop through `state.read_folders` before the guard runs)
and private copies (`forked_from` in the fork sidecar), and answering with the
reference LIST rather than prose so the tab can offer a jump to each. A package, runtime, markdown or private-copy spec is
