...
async def mark_arrival_folder_shared(state: "DashboardState", folder_id: str) -> None:
    ...
    Never raises, and never retried. A write that fails leaves the row unmarked,
    so a concurrent creator's rollback may reclaim it and this session renders at
    the top level -- the same outcome as the folder having been deleted, which
    the import handler's own repair already produces. The opposite direction
    would be to keep a row nothing is filed into, so failing toward the
    recoverable state is deliberate.
    """
    if not folder_id:
        return
