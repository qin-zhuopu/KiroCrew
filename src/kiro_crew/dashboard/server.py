...
async def _stt_startup_prewarm() -> None:
    ...
    * It never DOWNLOADS. Only an already-present model is warmed, checked with
      ``is_present`` before the engine is asked for anything. A gateway that pulls
      1.6 GB because it booted would be spending a user's bandwidth on a feature they
      have not used yet, and the first-run download stays where it is: an explicit
