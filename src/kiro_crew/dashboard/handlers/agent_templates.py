...
* ``GET /api/agents/templates`` -- every installed global template with the two
  ...
  (``used_by``: crews, the default agent, schedules, chat folders, private
  copies).
...
#: ``(folder_id, folder_name, default_agent)`` for one chat folder -- the
#: loop-side snapshot of the folder store the executor thread reads.
...
def template_references(
    ...
    ``schedule``, ``folder``, ``private_copy``. The list doubles as the delete
    guard's evidence and the roster's ``used_by``. A chat folder counts because
    its ``default_agent`` is what every new session filed there starts on; a
    template deleted underneath it would fall through to the default agent
    silently, the exact failure the guard exists to refuse.
    """
...
async def _folder_pins(state: DashboardState) -> list[FolderPin]:
    ...
    return await state.read_folders(
        lambda folders: [
...
def _template_rows(folders: list[FolderPin]) -> list[dict[str, Any]]:
    ...
    for info in infos:
        ...
        row["used_by"] = template_references(aliases, cfg, forks, crons, folders)
        rows.append(row)
...
async def api_agent_template_create(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        # is a 409 naming the problem, not a 500.
        try:
            if source_name:
                source, _resolved, taken, _path = await asyncio.to_thread(
                    _load_template_specs,
                    agents_dir,
                    source_name.strip(),
        ...
        def _create() -> Path:
            ...
            def _check_then_write(cfg_data: dict) -> None:
                if name.lower() in _reserved_binding_names(cfg_data):
                    raise _NameBound()
                if name in (cfg_data.get("agents") or {}):
                    raise _NameBound()
            ...
            update_config_locked(mutate=_check_then_write)
            return created[0]
...
async def api_agent_template_delete(request: web.Request) -> web.Response:
    ...
    while a crew, the default agent, a schedule, a chat folder or a private copy
    still names it: deleting underneath them would leave sessions that cannot
    start, or that silently start on the default agent.
    """
    ...
    async with _get_config_lock():
        ...
        def _guard_then_unlink() -> list[dict[str, str]]:
            cfg = KiroCrewConfig.load()
            aliases = {name, found.name, Path(found.filename).stem}
            refs = template_references(
                aliases, cfg, agent_state.all_fork_info(), _cron_agent_ids(), folders
            )
            if refs:
                return refs
            agents_dir = kiro_agents_dir_path()
            path = agents_dir / found.filename
            with agents_spec_lock(agents_dir):
                path.unlink()
        ...
        try:
            refs = await asyncio.to_thread(_guard_then_unlink)
