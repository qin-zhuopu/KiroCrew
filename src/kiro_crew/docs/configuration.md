...
| `stt.model` | Which speech model the local provider downloads and runs: `base-q8_0` (82 MB, fastest), `base` (148 MB), `small-q5_1` (190 MB, better at non-English), or `large-v3-turbo` (1.6 GB, most accurate, and the best choice for multilingual and mixed-language speech — but slower than real time without GPU acceleration). A superseded name such as `tiny` or `small` still loads, resolving to the surviving model in its size class | `"base"` |
